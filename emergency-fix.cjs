...
const drugContent = fs.readFileSync('src/components/dashboard/DrugSearch.tsx', 'utf8');
const FIXED_DRUG = drugContent
  .replace(
    /<td className="p-3 font-bold text-blue-700">\{item\.productName\}<\/td>/g,
  ...
  .replace(
    /\{item\.priceLabel \|\| '-'\}/g,
    '{ (item.priceLabel || "-").split("/")[0].trim() }'
  );
fs.writeFileSync('src/components/dashboard/DrugSearch.tsx', FIXED_DRUG);
...
`;
fs.writeFileSync('src/components/dashboard/Emergency.tsx', emContent);
