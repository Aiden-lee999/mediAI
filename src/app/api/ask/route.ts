  ...
  try {
    ...
    messages.push({
      ...
      content: [
        ...
        "[",
        "  { \"block_type\": \"textbook\", \"title\": \"1. 궁금한 약물 개요\", \"body\": \"약물의 작용 기전, 적응증 등 상세 설명\", \"meta_json\": { \"image_url\": \"제공된 사진 URL (없으면 빈 문자열)\" } },",
        "  { \"block_type\": \"textbook\", \"title\": \"2. 대체 옵션 논리\", \"body\": \"대체제 고려 사항 및 특징 설명\" },",
        "  { \"block_type\": \"drug_cards\", \"title\": \"3. 연관/대체 약물 (확장형)\", \"meta_json\": { \"drugs\": [ {\"name\":\"약명\",\"ingredient\":\"성분\",\"price\":\"단가 (API 정보 없으면 '확인요망' 기재. 임의창작 금지)\",\"class\":\"분류 (모르면 '확인요망')\",\"company\":\"제약사\"} ] } },",
        ...
        "]",
        "(*주의: JSON은 절대 마크다운 코드 블록 안에 넣지 마세요! 제공받은 API 데이터에 없는 단가/분류는 임의로 지어내지 말고 반드시 '확인요망'으로 적으세요! 이미지 경로도 있는 그대로만 쓰고 거짓 URL은 비워두세요!)",
        "",
        "**[최초의 약물 질문과 후속 질문 구분]**",
        "현재 대화는 " + (history && history.length > 1 ? "'후속 이어지는 대화'입니다." : "'최초의 첫 번째 질문'입니다."),
        (history && history.length > 1) 
          ? "👉 사용자와 이미 대화 중이므로, 추가적인 약물 검색이나 상세 정보 카드가 꼭 필요하지 않으면 Part 1(자연스런 서술)만 출력하고 짧게 답변을 종료하세요. (이 경우 ___JSON_BLOCKS___를 출력하지 마세요)" 
          : "🚨 현재 대화의 첫 번째 메시지이므로, 질문이 아무리 짧아도('~용량을 늘려달래' 등) 반드시 해당 약물을 유추하여 Part 1(서술)과 Part 2(___JSON_BLOCKS___ 및 8가지 JSON 배열 블록)를 모두 완벽하게 출력하세요!",
        "",
        drugContext
      ].join('\n')
    });
