  ...
  try {
    ...
    messages.push({
      ...
      content: [
        ...
        "Part 1 작성이 끝나면, 반드시 정확히 '___JSON_BLOCKS___' 라는 구분자를 출력하고, 그 아래에 프론트엔드가 UI 카드 및 이미지를 렌더링할 수 있도록 **해당 약물과 관련된 구조화된 JSON 배열**만 출력하세요. (절대 마크다운 코드블록 기호 등을 쓰지 마세요)",
        "",
        "**[최초의 약물 질문과 후속 질문 구분]**",
        "* **초기 질문(새로운 약물 혹은 증상):** Part 1(대화 서술) + Part 2(JSON 블록) 모두 출력.",
        "  - JSON은 절대 마크다운 코드 블록 등 기호 안에 쓰지 마시고, 바로 [ 기호로 시작하는 배열을 출력하세요.",
        "  - JSON 배열에는 다음 블록들이 반드시 포함되어야 합니다:",
        "    1. block_type: textbook (약물 개요 파트, meta_json 안에 image_url 키로 실시간 사진 URL 제공)",
        "    2. block_type: drug_cards (관련/대체 옵션 약물 리스트 파트, meta_json 안에 drugs 배열. 최대 5개 리스트만 작성! 5개가 넘어가면 스트리밍이 다운됩니다)",
        "    3. block_type: md_tip (처방 실무 팁)",
        "* **이어서 하는 대화(빠른 후속 질문):** Part 1(대화 서술)만 출력하고 바로 종료하세요. '___JSON_BLOCKS___' 나 표 등은 절대로 출력하지 마세요. 의사의 질문에 맞장구치듯 짧고 명확하게 답변만 던지세요.",
        "",
        drugContext
      ].join('\n')
    });
